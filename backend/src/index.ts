...
async function sendGameState(io: Server, roomId: string) {
    try {
        const [rooms] = await pool.query<any[]>('SELECT * FROM rooms WHERE id = ?', [roomId]);
        ...
        const [questions] = await pool.query<any[]>('SELECT * FROM questions WHERE room_id = ? ORDER BY id ASC', [roomId]);
